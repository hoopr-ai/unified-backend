...
// Fields that need integer conversion (round decimals)
const INTEGER_FIELDS = ["duration", "size"];
...
function mapSourceTrackToModel(sourceTrack: any): Partial<TrackModel> {
  ...
  for (const field of TRACK_MODEL_FIELDS) {
    ...
    // Handle integer fields (round decimals to integers)
    if (INTEGER_FIELDS.includes(field)) {
      if (sourceValue === null || sourceValue === undefined) {
        mappedTrack[field] = null;
      } else {
        const num = parseFloat(sourceValue);
        mappedTrack[field] = isNaN(num) ? null : Math.round(num);
      }
      continue;
    }
...
async function migrateTracks() {
  ...
  // Register TrackModel and its associated models with this Sequelize instance
  targetSequelize.addModels([TrackModel, TrackArtistMappingModel, ArtistModel]);
