...
async function migrateUserLicenses() {
  ...
  try {
    ...
    console.log(`✅ Found ${txRes.rows.length} TransactionInfo rows from source`);
    ...
    for (const row of txRes.rows) {
      ...
      if (!track_code) {
        console.warn(`⚠️  Skipping order_id=${order_id}, sku_id=${sku_id} — no trackCode`);
        skipped++;
        continue;
      }
      ...
      if (trackCheck.rows.length === 0) {
        console.warn(`⚠️  trackCode "${track_code}" not found in target tracks. Skipping.`);
        skipped++;
        continue;
      }
      ...
      for (let i = 0; i < licenseCount; i++) {
        const insertLicense = await target.query(`
          ...
          RETURNING id
        `, [brandId, TARGET_USER_ID, track_code, 1, licensed_at, licensed_at, licensed_at]);
      ...
      for (const vl of videoLinks) {
        const vlStatus = vl.status === "APPROVED" ? "APPROVED" : (vl.status || "ACTIVE");
        const vlTrackCode = vl.trackCode || track_code;

        await target.query(`
          INSERT INTO video_links ("licenseId", "url", "status", "trackCode", "createdAt", "updatedAt")
          VALUES ($1, $2, $3, $4, $5, $6)
        `, [firstLicenseId, vl.url, vlStatus, vlTrackCode, vl.createdAt || licensed_at, vl.createdAt || licensed_at]);
