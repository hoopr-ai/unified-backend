...
import { FaqModel } from "../services/persistence-service/faq/modules.export";
...
sequelize.addModels([FaqModel]);

// ── All FAQs from the Hoopr Smash FAQ document ──────────────────────────────

const FAQS = [
  // ── Credits & Usage Links ──────────────────────────────────────────────────
  {
    section: FaqSection.CREDITS_AND_USAGE_LINKS,
    order: 1,
  ...
  // ── Platform Usage ─────────────────────────────────────────────────────────
  {
    section: FaqSection.PLATFORM_USAGE,
    order: 9,
  ...
  // ── Music Usage ────────────────────────────────────────────────────────────
  {
    section: FaqSection.MUSIC_USAGE,
    order: 10,
  ...
  // ── Licensing Terms ────────────────────────────────────────────────────────
  {
    section: FaqSection.LICENSING_TERMS,
    order: 22,
  ...
  try {
    ...
    for (const faq of FAQS) {
      const [, created] = await FaqModel.findOrCreate({
        where: {
          platform: Platform.SOUND_TRACKING_APP,
          section: faq.section,
      ...
      if (created) {
        inserted++;
        console.log(`✅ [${faq.order}] Inserted: ${faq.question.slice(0, 60)}...`);
      } else {
        skipped++;
        console.log(`⏭️  [${faq.order}] Already exists, skipped`);
      }
    }
