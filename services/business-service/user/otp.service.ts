...
// Redis key prefixes
const KEY_OTP = (mobile: string) => `otp:${mobile}`;
const KEY_RESEND_ATTEMPTS = (mobile: string) => `otp:resend_attempts:${mobile}`;
const KEY_VERIFY_ATTEMPTS = (mobile: string) => `otp:verify_attempts:${mobile}`;
const KEY_BLOCK = (mobile: string) => `otp:block:${mobile}`;
...
export const sendOtpService = async (
  ...
  await redisClient.del(KEY_OTP(mobile));

  // Generate and store new OTP
  const otp = generateOtp();
  await redisClient.set(KEY_OTP(mobile), otp, "EX", OTP_TTL_SECONDS);

  // Increment resend counter
  await redisClient.set(
    KEY_RESEND_ATTEMPTS(mobile),
...
export const verifyOtpService = async (
  ...
  const isBlocked = await redisClient.get(KEY_BLOCK(mobile));
  if (isBlocked) {
    throw new AppError(
  ...
  // Get stored OTP
  const storedOtp = await redisClient.get(KEY_OTP(mobile));
  if (!storedOtp) {
  ...
  if (storedOtp === otp) {
    // OTP valid — clean up
    await redisClient.del(KEY_OTP(mobile));
    await redisClient.del(KEY_VERIFY_ATTEMPTS(mobile));
    await redisClient.del(KEY_RESEND_ATTEMPTS(mobile));
  ...
  const attempts = parseInt(await redisClient.get(KEY_VERIFY_ATTEMPTS(mobile)) || "0", 10) + 1;

  if (attempts >= MAX_VERIFY_ATTEMPTS) {
    // Block the number
    await redisClient.set(KEY_BLOCK(mobile), "1", "EX", BLOCK_DURATION_SECONDS);
    await redisClient.del(KEY_OTP(mobile));
    await redisClient.del(KEY_VERIFY_ATTEMPTS(mobile));
