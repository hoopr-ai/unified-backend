...
const ASSORTMENT: AssortmentItem[] = [
  ...
  {
    id: "hoopr originals",
    name: "Hoopr Originals",
    slug: "hoopr-originals",
    rank: 4,
  },
];

export const getAllFiltersService =
  async (): Promise<GetAllFiltersResponse> => {
    ...
    for (const filter of filters) {
      ...
      (groupedFilters[type] as FilterItem[]).push({
        id: filter.id,
        name: filter.name,
    ...
    for (const type of Object.keys(groupedFilters)) {
      (groupedFilters[type] as FilterItem[]).sort((a, b) => {
